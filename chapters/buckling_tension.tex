...
\todo{Mode number vs. $\mu,\lambda$ plot, and some discussion.}
...
\begin{equation}
\label{eq:tc_lomu}
t_c = \left(\frac{1}{2^{2/3}} + 2^{1/3}\right)\mu^{2/3}\lambda_{uu}^{1/3}
\end{equation}
...
Minimiizing \eqref{eq:tcn_hi_k} with respect to $\bar{n}$ gives the scaling law $\bar{n}=(\lambda_{uu}^{el})^{1/4}$. Inserting into \eqref{eq:tcn_hi_k} gives $t_c = 2\lambda_{uu}^{1/2}$. In terms of the net radial tension per unit length, $\bar{T}$, this gives
\begin{equation}
\label{eq:Tc_hi_k}
