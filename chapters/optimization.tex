...
Given the existence of an optimum $f_{rim}$ for a given $\chi_{extent},\chi_{geom},\chi_{matl}$, how does the wheel strength scale with size and mass?

Figure \ref{fig:opt_1D_scaling} (a) and (b) show how the strength varies with radius or mass, holding the other fixed. At each value of $R$ or $M$, the optimum rim mass fraction is calculated
