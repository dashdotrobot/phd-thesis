...
\cite{Keller2013}   & bicycle wheel    & spoke lacing pattern  & Weighted average of lateral stiffness and rotational stiffness &
   Rigid rim, linear-elastic spokes$^{\rm a}$ & brute-force search over hypercube\\
\cite{Svensson2015} & bicycle wheel    & spoke lacing pattern  & Multi-objective: stiffness and peak spoke force &
	Rigid rim, linear-elastic spokes$^{\rm b}$ & Genetic algorithm NSGA-II and NSGA-III \cite{Deb2002,Deb2014}\\
...
\hline
\end{tabularx}
\raggedright{$^{\rm a}$Using equations developed by Goldberg \cite{Goldberg1984}.}\\
\raggedright{$^{\rm b}$Using a video-game multiphysics engine.}\\
