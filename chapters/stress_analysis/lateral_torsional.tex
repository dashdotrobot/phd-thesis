...
% Describe offset spoke arrangements
...
% Stiffness trade-offs and mechanism rotation mode
...
where $y_{cr}$ is the height of the effective rotation center above the rim shear center. For a wheel with negative spoke offset, $y_{cr}>0$, (i.e. ``inside'' the wheel). Due to the coupling between bending and torsion under lateral loads, the rim also has a preferential rotation center which is only a function of the stiffness ratio, $\m$.
\begin{equation}
