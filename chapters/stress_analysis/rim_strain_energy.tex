...
After deformation, each point in the body displaces by \gls{uvec} and the basis vectors $\eo$, $\et$, $\eh$ rotate through angles $\omega_1$, $\omega_2$, $\omega_3$ to $\eo^*$, $\et^*$, $\eh^*$. The displacement vector at a point $(x, y, 0)$ in the cross-section is given by
\begin{equation}
\label{eq:u}
...
\begin{equation}
\label{eq:U_rim_gen}
U_{rim} = \frac{1}{2}\int_0^{2\pi \R} \int_{\gls{Arim}} (E\varepsilon_{33}^2 + G\gamma^2) \,d\gls{Arim}\,\, d\gls{arc}
\end{equation}
...
U_{rim} = \frac{1}{2}\int_0^{2\pi \R}
	...
	\EIl \left(u'' + \frac{\gls{p}}{\R}\right)^2 +
	EI_w\left(\gls{p}'' - \frac{u''}{R}\right)^2 +
	\GJ\left(\gls{p}' - \frac{u'}{R}\right)^2 ds
\end{multline}

Equation \eqref{eq:U_rim_uvw} is derived with the help of the following relations:
\begin{gather}
\int x\, d\gls{Arim} = \int y\, d\gls{Arim} = \int xy\, d\gls{Arim} = 0\\
\int d\gls{Arim} = \gls{Arim},\,\,\, \int x^2\, d\gls{Arim} = I_2,\,\,\, \int y^2\, d\gls{Arim} = I_1,\,\,\,
...
\begin{equation}
\label{eq:U_rim}
U_{rim} = \frac{1}{2}\int_0^{2\pi R} EA\varepsilon_c^2 + \EIr\kappa_1^2 + \EIl\kappa_2^2 + \GJ\kappa_3^2 + EI_w(\kappa_3')^2\, ds
\end{equation}
