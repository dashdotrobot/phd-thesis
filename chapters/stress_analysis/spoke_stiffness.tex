...
U_{spoke}^{\delta} = \frac{1}{2}(\delta\gls{usvec} \mathbf{k}_f \delta\gls{usvec}) +
    \p(\bs\times\eh)\mathbf{k}_s\delta\gls{usvec} +
    \frac{1}{2}\p^2(\bs\times\eh)\mathbf{k}_s(\bs\times\eh)
\end{equation}

Next we define an augmented shear center displacement vector and augmented spoke stiffness matrix:
\begin{gather}
\mathbf{d} = [\delta u_s, \delta v_s, \delta w_s, \p]^T\label{eq:d}\\
\mathbf{k}_s =
\begin{bmatrix}
...
\begin{equation}
\label{eq:U_spokes}
U_{spokes} = U_{spokes}^p + \sum_i^{n_s} \left(\gls{fs}_p^i \cdot \delta\gls{unvec}^i +
