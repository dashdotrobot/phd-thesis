...
U_{spoke} &= \int_{\mathcal{S}_0}^{\mathcal{S}_d} \mathbf{f} \cdot d\mathbf{u}_n\\
          ...
          &= U_{spoke}^p + \mathbf{f}_p \cdot \delta\mathbf{u}_n + \delta U_{spoke}^{pd}
\end{split}
\end{align}
...
The displacement $\delta\mathbf{u}_n$ can be decomposed into a component parallel to the spoke axis and a component transverse to the spoke axis. The parallel component leads to a force change in the axial direction $K_s \delta u_{n\parallel}$, where $K_s$ is the axial stiffness of the spoke. The transverse component produces a net restoring force in the transverse direction of $(T_p/l_s) \delta u_{n\perp}$ due to the rotation of the spoke through an infinitesimal angle $u_{n\perp}/l_s$. This is the same effect (tension stiffness, or membrane stiffness) which gives a tensed string or thin, taught membrane its transverse stiffness.
\begin{equation}
\label{eq:fs_us}
\delta\mathbf{f}_s =
...
\delta U_{spoke}^{pd} = \frac{1}{2}(\delta\mathbf{u}_s \mathbf{k}_f \delta\mathbf{u}_s) +
...
\delta\bar{U}_{spokes}^{pd} = \frac{1}{2}\int_0^{2\pi R} \mathbf{d}^T\, \mathbf{\bar{k}}\, \mathbf{d} \, ds
\end{equation}
