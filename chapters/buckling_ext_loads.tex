...
\begin{table}
\caption{Properties of the wheels in this study. Linear dimensions are given in millimeters. $\EIr$, $\EIl$, and $\GJ$ are given in N-m$^2$.}
...
\begin{equation}
\label{eq:Pc_lat}
P_{sb} = \frac{K_{lat}(\T) \T}{\gls{Ks} c_1}
\end{equation}
...
\begin{equation}
\label{eq:P_sb}
P_{sb} = \frac{K_{rad}\T}{\gls{Ks} c_2}
\end{equation}

where $K_{rad}$ is the radial stiffness of the wheel, $\gls{Ks}$ is the elastic stiffness of the spoke, and $c_2$ is the direction cosine of the spoke in the $\et$ (radial) direction. Unlike the lateral stiffness, the radial stiffness is not significantly sensitive to spoke tension.
...
\begin{equation}
\label{eq:T_nb}
\T^* = \frac{\T_c}{\left(1 + \frac{K_{rad}\T_c}{K_{lat}^0 \gls{Ks} R c_2}\right)}
\end{equation}
...
\begin{equation}
\label{eq:P_c_nb}
P_c = K_{lat}^0 R \left(\frac{1}{1 + \frac{\gls{Espk}\gls{As}}
...
In order to compare the experimentally-determined radial strength with Equation \eqref{eq:P_c_nb}, it is necessary to determine the rim radius $R$, the effective spoke axial stiffness $\gls{Espk}\gls{As}$, the lateral stiffness at zero spoke tension $K_{lat}^0$, the radial stiffness $K_{rad}$, and the critical buckling tension $\T_c$.
