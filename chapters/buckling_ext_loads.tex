...
\caption{Buckling under lateral force. \textbf{(a)} Snapshot from ABAQUS simulation showing the buckled shape. \textbf{(b)} Equilibrium load-displacement curves for a vintage road bike wheel for several spoke tensions. Lightest: $T=0.1T_c$, darkest: $T=0.6T_c$. \textbf{(c)} Failure diagram for the same wheel showing the region of no spoke buckling (green), buckled spokes but positive stiffness (yellow), and wheel collapse (red). The markers are from ABAQUS simulations and the dashed line is from Eqn. \eqref{eq:Pc_lat}.}
