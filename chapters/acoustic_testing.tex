...
\todo{Refer to earlier chapter on equivalent spring model rather than referencing conference paper.}Comparing Eqn. \eqref{eq:keff} with Eqns. 34 and 35 in reference \cite{Ford2016}, it's clear that the effective stiffness is twice the series combination of the rim bending stiffness and torsion stiffness. Even if $EI_{2}$ and $GJ$ cannot be reliably determined independently, the series combination can be directly determined from the relation $f_n^{lat} = (2\pi)^{-1}\sqrt{K_{rim,n}/M}$.
